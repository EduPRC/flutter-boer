...
class HomeState extends State<Home> {
  ...
  void calcularIMC() {
    double slr = double.parse((_salarioController.text).replaceAll(',', '.'));
    double result;
    double imposto;
    {
      if (slr <= 2000.00) {
        result = slr;
      } else if (slr >= 2001.00 && slr < 4000.00) {
        imposto = slr * 0.10;
        result = slr - imposto;
      } else if (slr >= 4001.00 && slr < 6000.00) {
        imposto = slr * 0.15;
        result = slr - imposto;
      }else {
        imposto = slr * 0.20;
    ...
    setState(() {
    _result = ("O valor do seu salario é ${(result.toStringAsFixed(2))}");
    });
  }
  }
  ...
  Widget buildCalcularButton() {
    return Padding(
      ...
      child: ElevatedButton(
        ...
        child: Text('Calcule o salario', style: TextStyle(color: Colors.red)),
      ),
    );
  }
